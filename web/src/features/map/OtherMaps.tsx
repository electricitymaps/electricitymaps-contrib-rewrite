...
export default function OtherMapPage(): ReactElement {
  console.log('Render');
  ...
  const { isLoading, isError, error, data: gridState } = useGetState(typedTimeAverage);
  const spatialAggregation = 'zone';
  const mapReference = useRef(null);

  const mapState = useMemo(() => getGeometries(), [spatialAggregation]);
  ...
  useEffect(() => {
    ...
    for (const [index, feature] of mapState.features.entries()) {
      const { zoneId } = feature.properties;
      const zone = gridState.data?.zones[zoneId];
  ...
  }, [mapReference, mapState, gridState, getCo2colorScale, datetimeIndex]);
